...
const mockMessages: Message[] = [
    { id: 1, text: "Olá, como está o projeto de robótica?", isUser: true, avatar: userAvatar },
    { id: 2, text: "Está indo muito bem! Terminamos a montagem.", isUser: false, avatar: friendAvatar },
    { id: 3, text: "Que ótimo! Precisamos de um novo desafio.", isUser: true, avatar: userAvatar },
    { id: 4, text: "Concordo. Já pensou em modelagem 3D?", isUser: false, avatar: friendAvatar },
    { id: 5, text: "Ainda não, mas é uma excelente ideia!", isUser: true, avatar: userAvatar },
    { id: 6, text: "Vou começar a pesquisar alguns tutoriais.", isUser: false, avatar: friendAvatar },
];

// --- Componente Balão de Chat ---
        ...
        <View style={[containerStyle]}>
            {!message.isUser && (
                <Image
                    ...
                    style={[bubbleStyles.avatar, { width: Math.round(maxWidth * 0.12), height: Math.round(maxWidth * 0.12), borderRadius: Math.round(maxWidth * 0.06) }]}
                    resizeMode="cover"
                />
            )}

            <View style={[bubbleStyle, { maxWidth }]}>
                <Text style={[bubbleStyles.text, textColorStyle]}>{message.text}</Text>
            </View>
        </View>
    );
};
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                <FlatList
                    ...
                    contentContainerStyle={[styles.chatListContent, { paddingTop: headerHeight + 8, paddingBottom: inputHeight + 12 }]}
                    style={styles.chatList}
                    inverted
                    keyboardShouldPersistTaps='handled'
                />

                {/* Campo de Entrada (fixo na parte inferior visualmente) */}
                <View style={[styles.inputArea, { height: inputHeight, paddingVertical: Math.round(inputHeight * 0.12) }]}>
                    <TouchableOpacity style={styles.micButton}>
                        <Icon name="mic-outline" size={Math.round(inputHeight * 0.4)} color="#000048" />
                    </TouchableOpacity>

                    <TextInput
                        style={[styles.textInput, { minHeight: inputHeight * 0.7, maxHeight: inputHeight * 1.5 }]}
                        value={inputText}
                    ...
                    <TouchableOpacity
                        style={[styles.sendButton, { width: Math.round(inputHeight * 0.85), height: Math.round(inputHeight * 0.85), borderRadius: Math.round(inputHeight * 0.85 / 2) }]}
                        onPress={handleSend}
                        disabled={!inputText.trim()}
                    >
                        <Icon
                            name="send"
                            size={Math.round(inputHeight * 0.38)}
